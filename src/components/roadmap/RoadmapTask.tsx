...
const RoadmapTask: React.FC<RoadmapTaskProps> = ({ task, completed, onToggle, roadmapId }) => {
  ...
  // Auto-generate YouTube search and web resources in the background when the user expands the task
  React.useEffect(() => {
    const triggerAutoGeneration = async () => {
      if (isExpanded && resources.length === 0 && !loading && !isGenerating && !autoGenerated) {
        ...
        try {
          ...
          setShowResources(true); // Auto-expand alternative trajectories list once loaded
          refetch();
        } catch (err) {
          console.error('Auto resource generation failed:', err);
          setAutoGenerated(false); // allow retry
        }
      }
    };
    triggerAutoGeneration();
  }, [isExpanded, resources.length, loading, isGenerating, autoGenerated, task, roadmapId]);

  // Find a video resource from the generated alternative resources if task.youtubeLink is empty
  const youtubeVideoResource = resources.find(r => r.resource_type === 'video');
  const hasYouTubeVideo = (task.youtubeLink && task.youtubeLink !== 'undefined') || !!youtubeVideoResource;
  const youtubeLink = task.youtubeLink && task.youtubeLink !== 'undefined' ? task.youtubeLink : youtubeVideoResource?.url;
  
  let youtubeThumbnail = task.youtubeThumbnail;
  if (!youtubeThumbnail && youtubeVideoResource?.url) {
    try {
      const urlObj = new URL(youtubeVideoResource.url);
  ...
  const youtubeTitle = task.youtubeTitle || youtubeVideoResource?.title || 'Recommended Reference Video';
      ...
      <div className={`group relative rounded-xl border transition-all duration-300 hover:bg-[#0C0C12]/40 ${
        ...
        <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
          <CollapsibleContent className="border-t border-white/[0.03] bg-[#050508]/30 px-4.5 sm:px-6 pb-5 pt-4 space-y-5">
            ...
            {isGenerating ? (
              <div className="text-left space-y-2 animate-pulse">
                <div className="flex items-center gap-1.5 text-[9px] font-mono text-slate-500 uppercase tracking-wider">
                  ...
                  Finding Video Tutorial...
                </div>
                <div className="bg-[#050508]/60 border border-white/[0.04] rounded-xl p-4 flex flex-col md:flex-row gap-5 items-stretch min-h-[140px]">
                  <div className="md:w-1/3 shrink-0 rounded-lg bg-white/5 min-h-[110px]" />
                  <div className="flex-1 flex flex-col justify-between space-y-3">
                    ...
                    <div className="h-8 bg-white/5 rounded w-28 self-end" />
                  </div>
                </div>
              </div>
            ) : hasYouTubeVideo ? (
                ...
                <div className="bg-[#050508]/60 border border-white/[0.04] rounded-xl p-4 shadow-lg hover:border-white/[0.08] transition-colors relative overflow-hidden flex flex-col md:flex-row gap-5 items-stretch">
                  ...
                  <div className="flex-1 flex flex-col justify-between space-y-3">
                    <div className="space-y-1.5">
                      <h4 className="text-sm font-bold text-white leading-snug line-clamp-2 font-display">
            ...
            <div className="text-left space-y-3">
              ...
              {isGenerating ? (
                <div className="bg-[#050508]/30 border border-purple-500/20 rounded-xl p-4 flex flex-col items-center justify-center py-6 text-center gap-3 animate-pulse">
                  <div className="w-8 h-8 rounded-full border-2 border-purple-500 border-t-transparent animate-spin" />
                  <div>
                    <p className="text-xs text-slate-300 font-semibold">Generating customized tutorials & resources...</p>
                    <p className="text-[10px] font-mono text-purple-400 mt-0.5">Searching YouTube and official documentations</p>
                  </div>
                </div>
              ) : totalAlternativeResources > 0 ? (
                showResources && (

...
async function checkRazorpaySubscription(razorpaySubscriptionId: string) {
...
async function updateSubscriptionInDB(supabase: any, userId: string, subscriptionData: any) {
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: subscriber } = await supabase
      .from('subscribers')
      .select('razorpay_subscription_id, subscribed, subscription_tier')
      .eq('user_id', user.id)
      .single();
    ...
    const razorpaySubscription = await checkRazorpaySubscription(subscriber.razorpay_subscription_id);
    
    // Update database with latest status
    const updatedSubscription = await updateSubscriptionInDB(supabase, user.id, razorpaySubscription);
